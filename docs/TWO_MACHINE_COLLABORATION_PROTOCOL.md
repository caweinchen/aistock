...
- 前端本地验证无误后，提交并先推送到 Gitee `main`，再同步 GitHub `main`，准备合并后端联调。
...
- 后端本地验证无误后，提交并先推送到 Gitee `main`，再同步 GitHub `main`，准备前端接入或联调。

## 双远端同步规则

- `gitee` 是主远端，`origin` 是辅助镜像远端。
- 默认本地 `main` 跟踪 `gitee/main`；如需切换跟踪目标，必须先获得用户明确授权。
- 验证通过后的提交，先推送到 `gitee`，再推送到 `origin`。
- `gitee` 推送失败时，不能把任务标记为已同步；必须先排查并重试。
- `origin` 推送失败时，需在相关计划、规格或看板的 TODO 中记录失败原因和待补动作，之后再补推。
...
8. 联调验证
9. 文档 TODO 状态更新
10. 提交并先推送 Gitee `main`，再同步 GitHub `main`
...
- Gitee 是主远端，固定名称为 `gitee`；GitHub 是辅助镜像，固定名称为 `origin`。
- 本地 `main` 默认跟踪 `gitee/main`，开始工作前优先从 Gitee 获取最新状态。
- 发现 Gitee 已有另一台电脑的新提交时，必须先安全 fetch 并集成，不得强推覆盖。
- 每次验证并提交后，按 `git push gitee main`、`git push origin main` 的顺序同步。
- Gitee 推送必须成功并核对远端 SHA，任务才可标记为“主远端已同步”。
- GitHub 同步失败时，必须报告准确错误并保留明确 TODO；允许继续协作，但不得声称“双端已同步”。
- 未经用户针对具体分支和已确认远端差异的明确授权，禁止任何强制推送。
- Multica workspace 和 agent 必须优先使用 Gitee 项目资源，避免同时登记两个内容相同的项目仓库造成选择歧义。
...
- 接口契约已更新。
- 没有未经授权的前端改动。
- 已提交并成功推送到 Gitee `main`；GitHub 已同步，或已记录明确的补推 TODO。
