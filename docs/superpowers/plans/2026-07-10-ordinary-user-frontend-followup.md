...
- [x] TODO: 确认前端类型中是否已有 `DataHealth`、`RiskExplanation`、`PreTradeChecklist` 和检查项状态。
- [x] TODO: 补齐或调整 API 归一化逻辑，让缺失新字段的旧响应可以回退到旧字段。
- [x] TODO: 在首页自选股洞察中展示数据健康概览。
- [x] TODO: 在个股详情普通用户摘要附近展示数据健康区块。
- [x] TODO: 在个股详情专业因子和策略数据之前展示风险说明书区块。
- [x] TODO: 实现买入前/卖出前检查清单 UI，用户确认状态只保存在本地 UI。
- [x] TODO: 检查前端文案，避免出现直接交易指令类表达。
- [x] TODO: 新增或更新前端测试，覆盖旧响应回退、首页数据健康、个股风险说明、检查清单交互。
- [x] TODO: 运行前端验证命令，并把结果记录回本计划。
...
- `npm test -- --runInBand`: 失败，当前 Vitest CLI 不支持 `--runInBand` 参数。
- `npm test`: 通过，10 个测试文件、29 个测试全部通过。
- `npm run typecheck`: 失败，项目未定义 `typecheck` 脚本。
- `npx tsc --noEmit`: 通过。
