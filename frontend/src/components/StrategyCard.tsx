...
import { translatePeriod, useTranslateRisk, useTranslateStrategyName, useTranslateStrategySummary, useTranslation } from '../i18n';
...
export function StrategyCard({ strategy, detail, isExpanded, isLoading, onPress }: StrategyCardProps) {
  ...
  const translateSummary = useTranslateStrategySummary();
  ...
  return (
    <Pressable style={({ pressed }) => [styles.strategyCard, pressed && styles.strategyCardPressed]} onPress={onPress}>
      ...
      <View style={styles.strategyStats}>
        <Metric label={translatePeriod(strategy.period, t)} value={formatPercent(strategy.return_rate)} suffix="" />
      ...
      <Text style={styles.strategySummary}>{translateSummary(strategy.summary)}</Text>
