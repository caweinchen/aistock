...
import { useTranslateFactor } from '../i18n';
...
export function FactorTile({ factor }: FactorTileProps) {
  ...
  const translateFactor = useTranslateFactor();
  ...
  return (
    <View style={styles.factorTile}>
      ...
      <Text style={styles.factorLabel}>{translateFactor.label(factor.key, factor.label)}</Text>
      <Text style={styles.factorDescription}>{translateFactor.description(factor.description)}</Text>
    </View>
  );
}
